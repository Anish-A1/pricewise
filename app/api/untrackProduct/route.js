...
export async function POST(req) {
  try {
    ...
    if (!trackedDocument) {
      return NextResponse.json(
        { error: "User not found or not tracking any products" },
        { status: 404 }
      );
    }

    // Find the tracked product in the trackedProducts array
    const productIndex = trackedDocument.trackedProducts.findIndex(
    ...
    if (productIndex === -1) {
      return NextResponse.json(
        { error: "Product not found in the tracked list" },
        { status: 404 }
      );
    }
